function imgs_correct = correctDistortion(images, v_intrinsic, varargin)
    ...
    defaultFillValue = -1000;
    % Specify the output view based on the size of the input image
    % according to Tipp: If the input transformation tform does not define a forward transform, 
    % then use the OutputView name-value argument to accelerate the transformation.
    R = imref2d(size(images{1}));
    switch distort_plane
        case 'pixel'
            inversefn = @(pts) distort_v1(pts, distort_coeff, distort_model, 'pixel');
            tform = geometricTransform2d(inversefn);
            for k=1:length(images)
                imgs_correct{k} = imwarp(images{k},tform,'FillValues', defaultFillValue,  'OutputView', R);   % <--- Using OutputView Might be wrong here??
                % Replace the fill value with NaN
                imgs_correct{k}(imgs_correct{k} == defaultFillValue) = NaN;
            end
        case 'normal'
            ...
            tform = geometricTransform2d(inversefn);
            for k=1:length(images)
                imgs_correct{k} = imwarp(images{k},tform,'FillValues', defaultFillValue,  'OutputView', R);   
                % Replace the fill value with NaN
                imgs_correct{k}(imgs_correct{k} == defaultFillValue) = NaN;
            end
        otherwise
