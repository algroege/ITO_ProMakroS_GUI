function perform_heterodyne_measurement_without_camera(app, dlg)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cameraSize = [2448 2050];
x_vec_detec = (1:cameraSize(1));
xy_mat_detec = ones(cameraSize(2),1) * x_vec_detec;

%% Allocate memory for frames to capture
imagesLam_01 = zeros(cameraSize(2),cameraSize(1),N);
...
for ii=0:N-1
    ...
    drawnow;
    pause(waitPeriode);
    filename = [folder '1_' num2str(ii) '.bmp'];
    imagesLam_01(:,:,ii+1) = imread(filename);
end
dlg.Value = 1/3;
% 2. Lambda 2
...
for ii=0:N-1
    ...
    drawnow;
    pause(waitPeriode);
    filename = [folder '2_' num2str(ii) '.bmp'];
    imagesLam_02(:,:,ii+1) = imread(filename);
end
dlg.Value = 2/3;
% 3. Lambda 3
...
for ii=0:N-1
    ...
    drawnow;
    pause(waitPeriode);
    filename = [folder '3_' num2str(ii) '.bmp'];
    imagesLam_03(:,:,ii+1) = imread(filename);
end
dlg.Value = 2.5/3;
stopCamera(app)
...
PHI_0avg_masked = PHI_0avg .* M_Mask;

% Phase to Height
z_data = app.phaseToHeigthFit(:,:,1) .* PHI_0avg_masked + app.phaseToHeigthFit(:,:,2);

% remove outliers
z_data_vec = z_data(:);
z_data_vec_orem = filloutliers(z_data_vec, "nearest");
z_data_orem = reshape(z_data_vec_orem, flip(cameraSize));

% 5. Correct for distortion
images = cell(1,1);
images{1} = z_data_orem;
images_undist = correctDistortion(images, app.calib_struct.intrinsics);
z_data_orem_undist = images_undist{1};
...
app.measurementResult.X = X;
app.measurementResult.Y = Y;
app.measurementResult.Z = z_data_orem_undist;
dlg.Value = 3/3;
end